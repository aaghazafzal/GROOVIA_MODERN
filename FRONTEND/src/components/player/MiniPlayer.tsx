...
const MiniPlayer = () => {
    ...
    const handleDownload = () => {
        ...
        if (isYoutube && currentSong.youtubeId) {
            // YT Music song — use Vercel scraper (primary)
    ...
    useEffect(() => {
        const fetchSongUrl = async () => {
            ...
            if (isYoutube) {
                // Vercel scraper is PRIMARY — yt-dlp pure Python, no JS engine needed
                // Backend is FALLBACK — Render gets blocked by YouTube on datacenter IPs
                    ...
                    onError={(e) => {
                        ...
                        if (currentSong?.youtubeId) {
                            if (audioUrl?.includes(YT_SCRAPER_URL)) {
                                // Vercel scraper failed → try backend as fallback
