    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // Search for "Most Streamed Love Songs Hindi" playlist
                const searchResponse = await api.get('/search/playlists', {
                    params: { query: 'Most Streamed Love Songs Hindi', limit: 5 }
                });
                ...
                if (results.length > 0) {
                    ...
                    const match = results.find((p: any) =>
                        ...
                        p.name?.toLowerCase().includes('most streamed love songs')
                    ) || results[0];

                    // Fetch playlist songs (28 songs - as per old website)
                    const playlistResponse = await api.get('/playlists', {
                        params: { id: match.id, limit: 28 }
                    });

                    if (playlistResponse.data.success && playlistResponse.data.data.songs) {
                        setSongs(playlistResponse.data.data.songs.slice(0, 28));
                    }
                }
            } catch (error) {
                console.error('Error fetching Love Songs:', error);
            } finally {
                setLoading(false);
            }
        };
        ...
        <>
            ...
            <div className="md:hidden py-3">
                <div className="flex justify-between items-center mb-3">
                    <h2 className="text-2xl font-bold text-white">Most Streamed Love Songs – Hindi</h2>
                </div>
            ...
            <div className="hidden md:block relative py-4 px-4">
                <div className="flex justify-between items-center mb-4">
                    <h2 className="text-3xl font-bold text-white">Most Streamed Love Songs – Hindi</h2>
